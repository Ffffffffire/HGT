...
def run_model_DBLP(args):
    ...
    for i in range(args.repeat):
        ...
        if args.model_type == 'gat':
            ...
            net = GAT(g, in_dims, args.hidden_dim, num_classes, args.num_layers, heads, F.leaky_relu, args.dropout, args.dropout, args.slope, True)
        ...
        with torch.no_grad():
            ...
            pred = test_logits.cpu().numpy().argmax(axis=1)
            onehot = np.eye(num_classes, dtype=np.int32)
            dl.gen_file_for_evaluate(test_idx=test_idx, label=pred, file_name=f"{args.dataset}.txt")
            pred = onehot[pred]
