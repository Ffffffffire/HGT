...
from dgl.nn.pytorch import GATConv, GraphConv, edge_softmax
...
class GCN(nn.Module):
    def __init__(self,
        ...
        for i in range(num_layers - 1):
            self.layers.append(GraphConv(num_hidden, num_hidden, activation=activation, bias = True))
        # output layer
    ...
    def forward(self, features_list):
        ...
        for i, layer in enumerate(self.layers):
            h = self.dropout(h)
            h = layer(self.g, h)
        #h = h / (torch.norm(h, dim=1, keepdim=True)+1e-12)
        return h
