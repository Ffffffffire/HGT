...
from GT import GT, RGT, RGT_v2, RGT_v3
...
def run_model_DBLP(args):
    ...
    train_nodes = np.arange(g.num_nodes())[train_idx]
    val_nodes = np.arange(g.num_nodes())[val_idx]

    '''

    for i in range(3):
        print("Train:", end = " ")
        seqs = node2seq(g, train_nodes, i+1)
        len_seq_min_max(seqs)
        print("Valid:", end = " ")
        seqs = node2seq(g, val_nodes, i+1)
        len_seq_min_max(seqs)

    '''

    
    #train_seq_r = dgl.sampling.random_walk(g, train_nodes, length=20)[0]
    #val_seq_r = dgl.sampling.random_walk(g, val_nodes, length=20)[0]


    train_seq = torch.zeros(len(train_idx), args.len_seq).long()
    val_seq = torch.zeros(len(val_idx), args.len_seq).long()
    ...
    for x in train_nodes:
        ...
        while (cnt < args.len_seq):
            ...
            for i in range(nsampled):
                train_seq[n, cnt] = sampled_list[i]
                cnt += 1
                if cnt == args.len_seq:
                    break
            scnt += 1
    ...
    for x in val_nodes:
        ...
        val_seq[n, cnt] = x
        cnt += 1
        start = val_seq[n, scnt].item()
        while (cnt < args.len_seq):
            sample_list = g.successors(start).numpy().tolist()
            nsampled = max(int(len(sample_list) * args.drop_ratio), 1)
            sampled_list = random.sample(sample_list, nsampled)
            for i in range(nsampled):
                val_seq[n, cnt] = sampled_list[i]
                cnt += 1
                if cnt == args.len_seq:
                    break
            scnt += 1
    ...
    for i in range(len(train_idx)):
        sg = dgl.node_subgraph(g, train_seq[i]).adj().to_dense().unsqueeze(0)
        train_adjs.append(sg)

    for i in range(len(val_idx)):
        sg = dgl.node_subgraph(g, val_seq[i]).adj().to_dense().unsqueeze(0)
        val_adjs.append(sg)

    train_adjs = torch.cat(train_adjs).to(device)
    val_adjs = torch.cat(val_adjs).to(device)


    #train_seq = torch.cat([train_seq, train_seq_r],dim=1)
    #val_seq = torch.cat([val_seq, val_seq_r], dim=1)
    ...
    for i in range(args.repeat):
        ...
        #net = GT(num_classes, in_dims, args.hidden_dim, args.ffn_dim, args.num_layers, args.num_heads, args.dropout)

        net = RGT_v3(num_classes, in_dims, args.hidden_dim, args.ffn_dim, args.num_layers, args.num_heads, args.dropout, args.rl_dim)
        net.to(device)
        ...
        for epoch in range(args.epoch):
            ...
            #logits = net(features_list, train_seq, args.usemean)
            logits = net(features_list, train_seq, type_emb, node_type, train_adjs, args.K)
            ...
            with torch.no_grad():
                ...
                #logits = net(features_list, val_seq, args.usemean)
                logp = F.log_softmax(logits, 1)
        ...
        with torch.no_grad():
            ...
            pred = onehot[pred]
            result = dl.evaluate_valid(pred, dl.labels_train['data'][val_idx])
            print(result)
