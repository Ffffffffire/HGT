...
def run_model_DBLP(args):
    ...
    dist_seq = torch.zeros(features_list[0].shape[0], args.len_seq).long()
    ...
    for i in range(args.repeat):
        ...
        net = GT(num_classes, in_dims, args.hidden_dim, args.ffn_dim, args.num_layers, args.num_heads, args.dropout,activation = 'leakyrelu', num_glo= args.num_g)
        ...
        for epoch in range(args.epoch):
            ...
            idx, y_a, y_b, alpha = mixup_data(train_seq.shape[0], labels[train_idx], args.alpha)

            logits = net.mixup_forward(features_list, train_seq, idx, alpha, args.usenorm)
            logp = F.log_softmax(logits, 1)
            train_loss = alpha * \
                F.nll_loss(logp, y_a) + (1-alpha) * F.nll_loss(logp, y_b)
