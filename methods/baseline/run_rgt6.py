...
def run_model_DBLP(args):
    ...
    for i in range(args.repeat):
        ...
        early_stopping = EarlyStopping(patience=args.patience, verbose=True,
                                       save_path='checkpoint/rgt_target_{}_{}.pt'.format(args.dataset, args.num_layers))
        ...
        net.load_state_dict(torch.load(
            'checkpoint/rgt_target_{}_{}.pt'.format(args.dataset, args.num_layers)))
        net.eval()
        with torch.no_grad():
            logits = net(features_list, test_seq, type_emb, node_type, test_adjs, args.K)
            test_logits = logits
            pred = test_logits.cpu().numpy().argmax(axis=1)
            dl.gen_file_for_evaluate(test_idx=test_idx, label=pred, file_name=f"{args.dataset}_{i}.txt")
