...
from GT import GT, RGT, RGT_v2, RGT_v3, RGT_v5
...
def run_model_DBLP(args):
    ...
    for i in range(args.repeat):
        ...
        net = RGT_v5(num_classes, in_dims, args.hidden_dim, args.ffn_dim, args.num_layers, args.num_heads, args.dropout, args.rl_dim)
        net.to(device)
        ...
        early_stopping = EarlyStopping(patience=args.patience, verbose=True,
                                       save_path='checkpoint/rgt_full_{}_{}.pt'.format(args.dataset, args.num_layers))
        for epoch in range(args.epoch):
            t_start = time.time()
            # training
            net.train()

            logits = net(features_list, train_idx, node_seq, type_emb, node_type, all_adjs, args.K)
            ...
            net.eval()
            with torch.no_grad():
                logits = net(features_list, val_idx, node_seq, type_emb,node_type, all_adjs, args.K)
        ...
        net.load_state_dict(torch.load(
            'checkpoint/rgt_full_{}_{}.pt'.format(args.dataset, args.num_layers)))
        net.eval()
        #test_logits = []
        with torch.no_grad():
            logits = net(features_list, test_idx, node_seq, type_emb, node_type, all_adjs, args.K)
            #logits = net(features_list, val_seq, args.usemean)
            #val_logits = logits
